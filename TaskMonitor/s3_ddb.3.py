...
def statRunByScanItems(profile='default',monitorTableName="s3cross_monitor",indexMonitor="ReplicationStatus-ReplicationTime-index",statTableName="s3cross_stat"):
    ...
    totalItems = [] 
    statMaxTime = getMaxStatTimestamp(1,profile,statTableName)
    ...
    logger.info("Start to query all items whose replication time is greater or equals with %d",beginTime)
    try:
        ...
        TIME_UNIT_ONE = "1Minute"
        TIME_UNIT_TEN = "10Minutes" 
        statMatrix = [
            {
        ...
        statItems = []  
        lenStatCategory = len(statMatrix)
...
def statRunByTimeRange(profile='default',monitorTableName="s3cross_monitor",statTableName="s3cross_stat"):
...
def private_getMonitorItems(startTime,endTime,replicationStatus='1',profile='default',tableName='s3cross_monitor',indexName='gsiStatus'):
    logger.info("*** DDB Query Request *** %d ,%d",startTime,endTime)
    session = boto3.Session(profile_name=profile)
    ddbClient = session.client('dynamodb')
    paginator = ddbClient.get_paginator('query')
    totalItems = [] 
    try:
        ...
        iterator = paginator.paginate(
            ...
            ,FilterExpression="ReplicationTime >= :startTime and ReplicationTime < :endTime"
            ,PaginationConfig={
                # 'MaxItems': 10,
                'PageSize': 10
            } 
